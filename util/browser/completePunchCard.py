    ...
    for childPromotion in childPromotions:
        ...
        if not childPromotion.complete:
            if childPromotion.promotionType == "urlreward":
                browser.find_elements(By.CLASS_NAME, "offer-cta")[1] \
            ...
            if childPromotion.promotionType == "quiz":
                continue
                browser.find_elements(By.CLASS_NAME, "offer-cta")[0] \
                    .find_element(By.TAG_NAME, "button") \
                    .click()  # click punch card button

                time.sleep(1)
                browser.switch_to.window(window_name=browser.window_handles[1])  # move browser to newly opened tab
