...
def exec_punch_cards(browser: WebDriver):
    ...
    if not punch_cards:
        ...
        logger.critical(
            "Unable to complete punch cards. Attribute is None or Empty Array "
    ...
    for punch_card in punch_cards:
        if (
            punch_card.parentPromotion is not None
            ...
            try:
                ...
                logger.critical(
                    "Failed to complete punch card child promotions. Expected error: "
                    f"Error report has been generated: {errorReport.file_path}"
                )
            else:
                logger.info(
                    f"Successfully completed punch card child promotion: {punch_card.name}"
                )
