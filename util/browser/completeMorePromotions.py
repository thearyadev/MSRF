...
def exec_additional_promotions(browser: WebDriver):
    ...
    if accountData is None:
        logging.critical(
            "Unable to complete more promotions due to missing dashboard data."
        )
        return
    ...
    if not more_promotions:
        errorReport: ErrorReport = ErrorReporter().generate_report(
            ...
            exception=Exception(
                "Manual exception. Missing more_promotions dashboard data"
            ),
        )
        logging.critical(
            "Unable to complete more promotions. Attribute is None or Empty Array"
        ...
        try:
            if not promotion.complete and promotion.pointProgressMax != 0:
                if promotion.promotionType == "urlreward":
                    ...
                    try:
                        util.complete_more_promotion_search(
                            browser=browser, cardNumber=cardNo
                        )
                        ...
                        logger.critical(
                            "Uncaught error in more promotions scraper. "
                    ...
                    if promotion.pointProgressMax == 10:
                        ...
                        try:
                            util.complete_more_promotion_abc(
                                browser=browser, cardNumber=cardNo
                            )
                        except Exception as e:
                            errorReport: ErrorReport = ErrorReporter().generate_report(
                        ...
                        try:
                            util.complete_more_promotion_quiz(
                                browser=browser, cardNumber=cardNo
                            )
                        except Exception as e:
                            errorReport: ErrorReport = ErrorReporter().generate_report(
                        ...
                        try:
                            util.complete_more_promotion_this_or_that(
                                browser=browser, cardNumber=cardNo
                            )
                        except Exception as e:
                            errorReport: ErrorReport = ErrorReporter().generate_report(
